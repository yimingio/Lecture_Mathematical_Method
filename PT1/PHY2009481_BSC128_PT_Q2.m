...
for i=1:4 
        xs=linspace(x1(i),x1(i+1),50);
        for j=1:50
            ys(j)=coeff1(i,1)+coeff1(i,2)*(xs(j)-x1(i))+coeff1(i,3)*(xs(j)-x1(i))^2+coeff1(i,4)*(xs(j)-x1(i))^3;
        end
        plot(xs,ys,'b');
        hold on
end
...
hold on
xt=linspace(-1,1,1000);
for i=1:1000
    yt(i)=func(xt(i));
end
plot(xt,yt,"Color",'k');

title("$f(x)=\frac{1}{1+25x^2}$(blue for 5 points,red for 9 points,black for actuall function)",'Interpreter','latex',"FontSize",20);
